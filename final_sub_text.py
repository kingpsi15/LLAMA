...
def find_subheading_by_name(subheadings, name):
    """
    Find a subheading dictionary by exact or partial matching of its text.
    """
    name_lower = name.lower()
    for sh in subheadings:
        if name_lower in sh["text"].lower():
            return sh
    return None

def extract_text_between_two_subheadings(lines, subheadings, start_name, end_name, pdf_path):
    """
    Extract text between two subheadings identified by their names using PyPDF text extraction.
    """
    start_sh = find_subheading_by_name(subheadings, start_name)
    end_sh = find_subheading_by_name(subheadings, end_name)

    if not start_sh or not end_sh:
        print(f"Could not find start or end subheading '{start_name}' or '{end_name}'")
        return []

    reader = PdfReader(pdf_path)

    start_page = start_sh["page"] - 1  # zero-based index for PyPDF
    end_page = end_sh["page"] - 1

    extracted_text = []

    # Extract start page text and trim after subheading text
    start_page_text = reader.pages[start_page].extract_text() or ""
    start_index = start_page_text.lower().find(start_sh["text"].lower())
    if start_index != -1:
        start_page_text = start_page_text[start_index + len(start_sh["text"]):].strip()
    extracted_text.append(start_page_text)

    # Full pages between start and end
    for p in range(start_page + 1, end_page):
        page_text = reader.pages[p].extract_text()
        if page_text:
            extracted_text.append(page_text.strip())

    # Extract end page text and trim before end subheading text
    end_page_text = reader.pages[end_page].extract_text() or ""
    end_index = end_page_text.lower().find(end_sh["text"].lower())
    if end_index != -1:
        end_page_text = end_page_text[:end_index].strip()
    extracted_text.append(end_page_text)
...
def save_text_to_file(text_lines, output_path):
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("\n".join(text_lines))
    print(f"Saved extracted text between subheadings to {output_path}")
...
def main():
    ...
    for i in range(len(excel_subheadings) - 1):
        start_subheading_name = excel_subheadings[i]
        end_subheading_name = excel_subheadings[i + 1]

        extracted_lines = extract_text_between_two_subheadings(grouped_lines, subheadings, start_subheading_name, end_subheading_name, pdf_path)

        if extracted_lines:
            start_word = start_subheading_name.split()[0].lower()
            end_word = end_subheading_name.split()[0].lower()

            output_file = f"extracted_{start_word}_to_{end_word}.txt"
            save_text_to_file(extracted_lines, output_file)

            print(f"Saved extracted text from {start_word} to {end_word}")
        else:
            print(f"No text extracted between {start_subheading_name} and {end_subheading_name}")
