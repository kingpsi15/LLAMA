...
def extract_text_between_two_subheadings(lines, subheadings, start_name, end_name):
    """
    Extract text between two subheadings identified by their names.
    """
    ...
    for line in lines:
        page = line[0]["page"]
        y = line[0]["y"]
        if (page > start_sh["page"] or (page == start_sh["page"] and y > start_sh["y"])) and \
           (page < end_sh["page"] or (page == end_sh["page"] and y < end_sh["y"])):
            text = "".join(char["text"] for char in line).strip()
            if text:
...
def main():
    ...
    print(f"{'Sl. No':<8} {'Name':<70} {'Page No'}")
    print("-" * 90)
    for idx, sh in enumerate(subheadings, start=1):
        print(f"{idx:<8} {sh['text']:<70} {sh['page']}")

    # Extract and save text between two specific subheadings (example)
    start_subheading_name = "SC-1 POLICY AND PROCEDURES"  # Change this to your actual starting subheading text
    end_subheading_name = "SC-2 SEPARATION OF SYSTEM AND USER FUNCTIONALITY"    # Change this to your actual ending subheading text

    extracted_lines = extract_text_between_two_subheadings(grouped_lines, subheadings, start_subheading_name, end_subheading_name)

    if extracted_lines:
        output_file = f"extracted_{start_subheading_name}_to_{end_subheading_name}.txt"
        save_text_to_file(extracted_lines, output_file)
    else:
        print("No text extracted between specified subheadings.")
